...
# PREP --------------
pkgs <- c('tidyverse','reshape2','purrr')
...
SBA_summary_cty <- SBA_matched %>%
  ...
  summarise(SBAamt.tot = sum(SBAGuaranteedApproval, na.rm = TRUE),
...
EXIM_tomatch <- EXIM %>%
  mutate(PLACE = trimws(toupper(Primary.Exporter.City)),
...
EXIM_summary_cty <- EXIM_city_matched %>%
  ...
  summarise(APPamt.tot = sum(Approved.Declined.Amount, na.rm = TRUE),
...
TLR_matched <- TLR %>%
  mutate(FIPS = padz(projectfipscode_2000,11),
  ...
  data <- tryCatch(read_delim(con1, col_names = FALSE, delim = " "), 
                   error = function(e){
                     tryCatch(read_delim(con2, col_names = FALSE, delim = " "),
                              error = function(c){read_delim(con3,col_names = FALSE, delim = " ")}
                              )}
                   )
...
FDIC_matched <- FDIC_interpret %>%
  # filter(CBSA!="NA")%>%
  # select(-X2,-X5) %>%
  mutate(n_tot = n_100k+n_250k+n_1m,
...
NMTC_matched <- NMTC %>%
  mutate(FIPS = padz(as.character(`2010 Census Tract`),11),
         county14 = as.numeric(substr(FIPS,1,5)))
